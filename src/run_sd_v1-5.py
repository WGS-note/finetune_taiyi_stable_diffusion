...
if __name__ == '__main__':
    ...
    gen_img(pipe=pipe, prompt=prompt, path='./gen_imgs/sd1-5-1.png')

    # 哈士奇和柯基生成的狗长什么样？
    prompt = "What is Husky and Corgi's child like?"
    gen_img(pipe=pipe, prompt=prompt, path='./gen_imgs/sd1-5-2.png')

    # 一只驾驶宇宙飞船的柯基
    prompt = 'A corgi piloting a spaceship'
    gen_img(pipe=pipe, prompt=prompt, path='./gen_imgs/sd1-5-3.png')

    # 君不见黄河之水天上来
    prompt = 'The water of the Yellow River comes from heaven'
    gen_img(pipe=pipe, prompt=prompt, path='./gen_imgs/sd1-5-4.png')

    # 君不见黄河之水天上来，Van Gogh style
    prompt = 'The water of the Yellow River comes from heaven, Van Gogh style'
    gen_img(pipe=pipe, prompt=prompt, path='./gen_imgs/sd1-5-5.png')

    # 君不见黄河之水天上来，科幻，国画
    prompt = 'The water of the Yellow River comes from heaven, science fiction, traditional Chinese painting'
    gen_img(pipe=pipe, prompt=prompt, path='./gen_imgs/sd1-5-6.png')

    # 一只穿着宇航服的哈士奇
    prompt = 'A husky in a space suit'
    gen_img(pipe=pipe, prompt=prompt, path='./gen_imgs/sd1-5-7.png')

    # 一只穿着宇航服的哈士奇，科幻
    prompt = 'A husky in a space suit, science fiction'
    gen_img(pipe=pipe, prompt=prompt, path='./gen_imgs/sd1-5-8.png')

    # 一只穿着宇航服的哈士奇，Van Gogh style
    prompt = 'A husky in a space suit, Van Gogh style'
    gen_img(pipe=pipe, prompt=prompt, path='./gen_imgs/sd1-5-9.png')

    # 一只吃胡萝卜的兔子正在发呆
    prompt = 'A rabbit eating a carrot is in a daze'
    gen_img(pipe=pipe, prompt=prompt, path='./gen_imgs/sd1-5-10.png')

    # 一只在吃白菜、头顶有帽子的小兔子
    prompt = 'A rabbit with a hat on his head eating cabbage'
    gen_img(pipe=pipe, prompt=prompt, path='./gen_imgs/sd1-5-11.png')
    ...
    prompt = '''
        ...
        '''
    gen_img(pipe=pipe, prompt=prompt, path='./gen_imgs/sd1-5-12.png')
    ...
    prompt = '''
        ...
        '''
    gen_img(pipe=pipe, prompt=prompt, path='./gen_imgs/sd1-5-13.png')

    # 纳尔应该怎么玩啊
    prompt = 'How should Gnar play'
    gen_img(pipe=pipe, prompt=prompt, path='./gen_imgs/sd1-5-14.png')
    ...
    gen_img(pipe=pipe, prompt=prompt, path='./gen_imgs/sd1-5-15.png')
